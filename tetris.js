...
const emptyBlock = {};
const redBlock = { color: 0xff0000 };
class Piece {
    constructor(blocks, pivot, color) {
        this.blocks = blocks;
        this.pivot = pivot;
        this.color = color;
    }
...
class Game {
    ...
    redraw() {
        ...
        g.drawRect(0, 0, sizes['fieldWidth'], sizes['fieldHeight']);
        g.endFill();
        // falling piece
        if (this.piece != undefined) {
            ...
            for (const i in blocks) {
                const x = blocks[i].x;
                const y = blocks[i].y;
                g.drawRect(x * sizes['outerSize'] + sizes['margin'], y * sizes['outerSize'] + sizes['margin'], sizes['outerSize'] - sizes['margin'], sizes['outerSize'] - sizes['margin']);
            }
            g.endFill;
        }
        // garbage
        ...
        if (this.state != 'falling') {
            ...
            if (this.state == 'gameover') {
                ...
                this.text = new PIXI.Text('Tap or press key', {
                    fontFamily: 'Georgia',
                    fontSize: Math.round(sizes['fieldWidth'] / 10),
                    fill: 0x141111
                });
